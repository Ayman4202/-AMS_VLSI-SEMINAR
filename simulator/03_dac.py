...
def plot_all(dac: DAC):
    ...
    # Formula
    ax_a.text(5.0, 4.5,
              r'$V_{CTRL} = \frac{code}{2^B} \times V_{ref}$'
              f'          $\\Delta V_{{LSB}}$ = {dac.dv_lsb*1e3:.3f} mV',
    ...
    ax_c.plot(codes_arr, np.zeros(dac.codes), color=C_DNL,
              linewidth=2.0, label='DNL = 0 LSB')
    ax_c.plot(codes_arr, np.zeros(dac.codes), color=C_INL,
              linewidth=1.5, linestyle='--', label='INL = 0 LSB')
    ax_c.axhline( 0.5, color=C_LIMIT, linewidth=0.9,
                 linestyle=':', label='±0.5 LSB limit')
    ax_c.axhline(-0.5, color=C_LIMIT, linewidth=0.9, linestyle=':')
    ax_c.axhline(0,    color='black',  linewidth=0.5)

    ax_c.text(dac.codes * 0.5, 0.62, '±0.5 LSB spec limit',
              ha='center', fontsize=7.5, color=C_LIMIT)
    ax_c.text(dac.codes * 0.5, 0.0,
              'Ideal DAC: DNL = INL = 0\nEvery step = exactly $\\Delta V_{LSB}$',
              ha='center', va='center', fontsize=8, color='gray',
              bbox=dict(boxstyle='round,pad=0.3', fc='#f5f5f5',
                        ec='black', lw=0.6, alpha=0.9))

    ax_c.set_xlabel('DAC Code')
    ax_c.set_ylabel('Error (LSB)')
    ax_c.set_xlim(-20, dac.codes + 20)
    ax_c.set_ylim(-1.5, 1.5)
    ax_c.legend(loc='upper right', fontsize=7.5)
    ax_c.set_title('(c)  DNL / INL  (Ideal)', loc='left',
                   fontsize=9, fontweight='bold')
    ...
    # Annotate code values on each step
    for i, (cy, code_v, v_v) in enumerate(zip(cycles, code_seq, v_seq)):
        if i < 12:
            y_off = 10 if i % 2 == 0 else -18
            ax_d.text(cy + 0.1, v_v + y_off, str(code_v),
    ...
    ax_d.set_title('(d)  Transient — $V_{CTRL}$ vs $CLKCTRL$ Cycle',
